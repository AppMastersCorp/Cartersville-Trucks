...
class CtTrucklistingController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $cttrucklisting = $query->paginate(20);
        ...
        return view('admin.cttrucklisting.index', compact('cttrucklisting', "ctcondition", "ctmake", "ctmodels", "ctregistered"));
    }
...
VALUES ('" . $bodytype_id_other . "','" . $now . "','" . $now . "') ");
    ...
    public function edit($id) {
        ...
        return view('admin.cttrucklisting.edit', compact('cttrucklisting', "ctcondition", "ctmake", "ctmodels", "ctregistered", "ctcolor", "ctexteriorcolor", "ctfueltype", "ctbodystyle", "cttransmission", "trucklisting_img", "enginesize", "horsepower", "torque", "drivetrain", "maxseating", "bodytype", "ctsubmodels"));
    }
...
VALUES ('" . $bodytype_id_other . "','" . $now . "','" . $now . "') ");
        ...
        $files_data = array();
         ...
         foreach ($files["tmp_name"] as $index => $value) {
             ...
             array_push($files_data, $this->create_photo($value, $files["name"][$index]));
        }
		die();
		...
		/*
         ...
         $files_data = array();
         foreach ($files["tmp_name"] as $index => $value) {
             $orig_name = $files["name"][$index];
             $orig_name = realpath(dirname(__FILE__) . "/" . basename($orig_name));
 
  

            $result = \Cloudinary\Uploader::upload($value, array(
                        "tags" => "ctrucks",
                        "public_id" => "ctrucks/" . $orig_name,
            ));
    ...
    public function create_photo($file_path, $orig_name) {

//        echo $file_path . '<br>';
//        echo $temp_file = tempnam(sys_get_temp_dir(), $orig_name);
//        die;
        # Upload the received image file to Cloudinary
       
		 

		/*$result = \Cloudinary\Uploader::upload(realpath(dirname(__FILE__).'\img.png') );*/
		
		/*
		$result =\Cloudinary\Uploader::upload(realpath(dirname(__FILE__).'\img.png'), array("public_id" => "sample_id", "tags" => "ctrucks"));
		*/
		
 
	   
	   /*
	   $result = \Cloudinary\Uploader::upload('C:\xampp\htdocs\framework\CTrucks\app\Http\Controllers\Admin\img.png');	   
	   */
	   
	   /*
	   $result = \Cloudinary\Uploader::upload(realpath($file_path), array(
                    "tags" => "ctrucks",
                    "public_id" => "ctrucks/" . $orig_name,
        ));
		*/
